 ...
 * author : Mashiro
 *
 * File name : unordered_set.hpp -a kind of unordered container.
 *
...
class unordered_set
{
	public:
		...
		using self = unordered_set<Ty ,Hash, alloc>;
	...
	public:
		...
		explicit unordered_set(size_type size)
		{
			ptr = new hash_table(size);
		}

		explicit unordered_set(const initializer_list<value_type>& list)
		{
		...
		unordered_set(iter beg , iter end)
		{
