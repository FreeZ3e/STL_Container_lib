 ...
 * author : Mashiro
 *
 * File name : unordered_multimap.hpp -a kind of unordered container.
 *
...
class unordered_multimap
{
	public:
		...
		using self = unordered_multimap<key , value , Hash , alloc>;
	...
	public:
		...
		explicit unordered_multimap(size_type size)
		{
			ptr = new hash_table(size);
		}

		explicit unordered_multimap(const initializer_list<value_type>& list)
		{
		...
		unordered_multimap(iter beg , iter end)
		{
		...
		unordered_multimap(const self& obj)
		{
		...
		{
			iterator pos = find(make_pair<key , value>(k));
