...
class thread_lock
{
	...
	public:
		...
		[[noreturn]] void insert(const value_type& elem)
		{
...
thread thread_push(lock& obj , const typename lock::value_type& elem)
{
	using func = void (lock::*)(const typename lock::value_type&);
	func push = &lock::insert;

	return thread(bind(push,&obj,elem));
}

template<typename lock>
thread thread_push(lock& obj , const initializer_list<typename lock::value_type>& list)
{
	using func = void (lock::*)(const initializer_list<typename lock::value_type>&);
	func push = &lock::insert;
