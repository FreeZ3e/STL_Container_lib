 ...
 * author : Mashiro
 *
 * File name : map.hpp -a kind of associative container.
 *
...
class map
{
	public:
		...
		using RB_Tree = RB_Tree<value_type , Unique_Compare , alloc>;
	...
	public:
		...
		explicit map(const_reference elem)
		{
			tree = new RB_Tree(elem);
			elem_count++;
		}

		explicit map(const initializer_list<value_type>& list) :map()
		{
			for (auto p : list)
				insert(p);
		}
		...
		map(iter beg , iter end) :map()
		{
			while (beg != end)
				insert(*beg++);
		}

		map(const self& obj) :map()
		{
		...
		_NODISCARD pair<iterator , bool> insert(const_reference elem)
		{
			...
			if (pos == end())
			{
				...
				return pair<iterator , bool>(find(elem.key) , true);
			}
		...
		{
			if (find(value_type(arg...).key) != end())
				return find(value_type(arg...).key);
