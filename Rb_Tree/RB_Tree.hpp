...
{	
	...
	public:
		...
		iterator erase(iterator& ptr)
		{
			...
			return iterator(Self_Search(num , head),step);
		}
		...
		const_iterator erase(const_iterator& ptr)
		{
			...
			return const_iterator(Self_Search(num , head) , step);
		}

		iterator erase(iterator& p_begin , iterator& p_end)
		{
		...
		{
			for (int n = p_begin.step(); n < p_end.step(); ++n)
			{
				p_begin = erase(p_begin);
			}
		}
		...
		//operator overload
		void operator=(const RB_Tree<Ty , Compare_Class>& obj)
		{
